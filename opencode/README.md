...
> Este repo también tiene un entorno para Claude Code, en [`../claude-code/`](../claude-code/README.md).
...
reproducible en cualquier máquina simlinkeando esta carpeta a `~/.config/opencode/`.

Esta carpeta **es** el directorio de configuración de OpenCode. No hay paso de build —
los archivos que están acá son lo que OpenCode carga al iniciar, una vez enlazada en su
lugar (ver Instalación más abajo).
...
En el repo esto vive en `opencode/` (junto a `claude-code/`, ver el README raíz). El
árbol de abajo muestra cómo queda **una vez enlazado** en su destino real:
...
# 2. Clonar el repo (en cualquier ubicación — ya no es la carpeta de config directamente)
git clone git@github.com:GianBaeza/IA-Workspace.git

# 3. Enlazar la carpeta opencode/ del repo en la ruta EXACTA que espera OpenCode
#    (hacer backup de un ~/.config/opencode existente primero)
ln -s "$(pwd)/IA-Workspace/opencode" ~/.config/opencode

# 4. Instalar dependencias npm (plugins)
cd ~/.config/opencode && npm install

# 5. Binarios externos referenciados por plugins/MCP:
...
`~/.config/opencode/`, así que el symlink (no una copia) es lo que hace que todos los
agentes, skills y plugins resuelvan correctamente — y que `git pull` en el repo se
refleje sin repetir el paso 3.

> Si ya tenías este repo clonado directo en `~/.config/opencode/` desde antes de esta
> reorganización: movelo o borralo, y volvé a crearlo como symlink apuntando a
> `IA-Workspace/opencode/` (paso 3). El contenido en sí no cambió, solo dónde vive
> dentro del repo.
